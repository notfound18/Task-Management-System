...
class TaskController {
  ...
  searchTasks(query) {
    try {
      ...
      const userResults = allResults.filter((task) => task.ownerId === this.currentUser.id || task.assigneeId === this.currentUser.id);
  ...
  getOverdueTasks() {
    try {
      ...
      const overdueTasks = this.taskRepository.findOverdue().filter((task) => task.ownerId === this.currentUser.id || task.assigneeId === this.currentUser.id);
  ...
  getTasksDueSoon(days = 3) {
    try {
      ...
      const dueSoonTasks = this.taskRepository.findDueSoon(days).filter((task) => task.ownerId === this.currentUser.id || task.assigneeId === this.currentUser.id);
